/* GPU version of the 4-point jacobi stencil to solve the Laplace equation.
 * Compile it with `gcc -o laplace.gpu 5_gpu.c image_ppm.c -O1 -lm -fopenmp -foffload=-misa=sm_35`.
 * Run it with `./laplace.gpu -i example_input.ppm`
 */
...
int laplace(FLT* U, unsigned int width, FLT dx, unsigned int height, FLT dy, int max_iter, FLT threshold) {
    if(U != NULL) {
        ...
        #pragma omp target data map(tofrom:U[0:width*height]) map(alloc:tmp[0:width*height])
        {
            FLT error = .0f;
            for(int iter=0; iter < max_iter; iter++) {
                error = .0f;
                
                #pragma omp target teams distribute parallel for simd reduction(max:error) map(tofrom:error)
                for(int y=1; y < (height - 1); y++) {
                    for(int x=1; x < (width - 1); x++) {
                        T(x, y) = (dy * dy * ( G(x+1, y) + G(x-1, y) ) +  dx * dx * ( G(x, y+1) + G(x, y-1) )) / (2 * dx * dx + 2 * dy * dy);
                        error = ffmax(error, fabs(G(x,y) - T(x,y)));
                    }
                }
